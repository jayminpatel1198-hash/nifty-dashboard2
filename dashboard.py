...
<title>Nifty Weight Dashboard V6</title>
<style>
...
<h2>NIFTY WEIGHTAGE DASHBOARD V6</h2>
...
<div class="card">Net Weight Score: <span class="big">{{ net_weight }}</span></div>
...
<div class="card">
<h3>Top Weight Contributors</h3>
<table><tr><th>Stock</th><th>Weight</th><th>% Change</th><th>Impact</th></tr>{{ contributor_rows }}</table>
</div>
...
def bucket_name(weight):
    if weight >= 10:
        return "10%+ Weight"
    elif weight >= 5:
        return "5% to 10% Weight"
    elif weight >= 1:
        return "1% to 5% Weight"
    else:
        return "Below 1% Weight"
...
def home():
    ...
    buckets = {
        "10%+ Weight": {"g":0,"r":0,"gw":0,"rw":0},
        "5% to 10% Weight": {"g":0,"r":0,"gw":0,"rw":0},
        "1% to 5% Weight": {"g":0,"r":0,"gw":0,"rw":0},
        "Below 1% Weight": {"g":0,"r":0,"gw":0,"rw":0},
    }
    ...
    for s in data.values():
        ...
        bucket = bucket_name(wt)

        if change > 0:
            green += 1
            green_weight += wt
            buckets[bucket]["g"] += 1
            buckets[bucket]["gw"] += wt
        elif change < 0:
            red += 1
            red_weight += wt
            buckets[bucket]["r"] += 1
            buckets[bucket]["rw"] += wt
        else:
            flat += 1
    ...
    call_probability = round((green_pct * 0.35) + (max(0, net_weight + 50) * 0.65), 1)
    put_probability = round((red_pct * 0.35) + (max(0, 50 - net_weight) * 0.65), 1)

    if green_weight > red_weight and red > green:
        pull_direction = "Heavyweights pulling market UP"
        reason = "Count red વધારે છે, પણ green weight વધારે છે."
    elif red_weight > green_weight and green > red:
        pull_direction = "Heavyweights pulling market DOWN"
        reason = "Count green વધારે છે, પણ red weight વધારે છે."
    elif green_weight > red_weight:
        pull_direction = "Market weight is bullish"
        reason = "Green sideનું total weight વધારે છે."
    elif red_weight > green_weight:
        pull_direction = "Market weight is bearish"
        reason = "Red sideનું total weight વધારે છે."
    else:
        pull_direction = "Balanced"
        reason = "Green અને red weight લગભગ same છે."

    if call_probability >= 75 and green_weight > red_weight:
        final_decision = "CALL BUY FAVOURABLE"
        entry_advice = "Only after breakout confirmation"
        option_risk = "LOW"
        color = "#d9fbe6"
    elif put_probability >= 75 and red_weight > green_weight:
        final_decision = "PUT BUY FAVOURABLE"
        entry_advice = "Only after breakdown confirmation"
        option_risk = "LOW"
        color = "#ffe1e1"
    elif green_weight > red_weight:
    ...
    top_contributors = sorted(contributors, key=lambda x: abs(x["impact"]), reverse=True)[:8]
    contributor_html = ""
    ...
    return render_template_string(
        ...
        bucket_rows=Markup(bucket_html),
        contributor_rows=Markup(contributor_html),
        gainers=make_simple_table(gainers),
        losers=make_simple_table(losers),
