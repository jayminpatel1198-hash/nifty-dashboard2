...
"HDFCLIFE":"NSE_EQ|INE795G01014","WIPRO":"NSE_EQ|INE075A01022"
}
...
<title>Nifty V20 Dashboard</title>
<style>
...
<h2>NIFTY 50 LIVE DASHBOARD V20</h2>
...
<h3>Clock Meter - Weightage Impact</h3>
<div class="gauge">
...
<div class="card">Positive Weight Effect: <span class="big green">+{{ positive_impact }}</span></div>
<div class="card">Negative Weight Effect: <span class="big red">{{ negative_impact }}</span></div>
<div class="card">Net Weight Effect: <span class="big">{{ net_impact }}</span></div>

<div class="card">
<h3>Top Pullers</h3>
<table><tr><th>Stock</th><th>Price</th><th>₹ Chg</th><th>% Chg</th><th>Weight</th><th>Effect</th></tr>{{ pullers }}</table>
</div>

<div class="card">
<h3>Top Draggers</h3>
<table><tr><th>Stock</th><th>Price</th><th>₹ Chg</th><th>% Chg</th><th>Weight</th><th>Effect</th></tr>{{ draggers }}</table>
</div>

<div class="card">
<h3>All 50 Stocks Price Change & Weightage Effect</h3>
<table><tr><th>Stock</th><th>Price</th><th>₹ Chg</th><th>% Chg</th><th>Weight</th><th>Effect</th></tr>{{ all_rows }}</table>
</div>
...
def make_rows(items):
    ...
    for r in items:
        ...
        out += f"<tr><td>{r['symbol']}</td><td>{r['price']}</td><td class='{rc}'>{r['rupee']}</td><td class='{cls}'>{r['pct']}%</td><td>{r['weight']}%</td><td class='{cls}'>{r['effect']}</td></tr>"
    return Markup(out)
...
def home():
    ...
    for s in data.values():
        ...
        if rupee > 0:
            green += 1
            positive_impact += effect
        elif rupee < 0:
            red += 1
            negative_impact += effect
        else:
            flat += 1
    ...
    total_effect = positive_impact + abs(negative_impact)
    weight_meter = round((positive_impact / total_effect) * 100,1) if total_effect > 0 else 50
    ...
    pullers = sorted([r for r in rows if r["effect"] > 0], key=lambda x:x["effect"], reverse=True)[:10]
    draggers = sorted([r for r in rows if r["effect"] < 0], key=lambda x:x["effect"])[:10]
