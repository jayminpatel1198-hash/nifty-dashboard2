...
weights = {"RELIANCE":9.47,"HDFCBANK":6.19,"BHARTIARTL":5.93,"SBIN":4.78,"ICICIBANK":4.75,"TCS":4.32,"BAJFINANCE":2.91,"LT":2.90,"INFY":2.64,"HINDUNILVR":2.62,"SUNPHARMA":2.29,"ADANIPORTS":2.21,"MARUTI":2.19,"AXISBANK":2.08,"ADANIENT":2.03,"KOTAKBANK":2.02,"M&M":2.00,"TITAN":1.93,"NTPC":1.90,"ITC":1.85}

HTML = """
<html><head>
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta http-equiv="refresh" content="5">
<title>Nifty Option Dashboard V5</title>
<style>
body{font-family:Arial;background:#f4f6f8;padding:14px;margin:0}
h2{text-align:center}.card{background:white;padding:15px;margin:10px;border-radius:14px;box-shadow:0 2px 5px #ddd}
.signal{padding:20px;border-radius:16px;text-align:center;font-size:25px;font-weight:bold;margin:10px}
.big{font-size:25px;font-weight:bold}.green{color:green;font-weight:bold}.red{color:red;font-weight:bold}
...
<h2>NIFTY OPTION DASHBOARD V5</h2>
...
def home():
    ...
    if green_pct >= 65 and bull_score >= 70:
        mood, strength, signal = "TRENDING BULLISH", "STRONG", "CALL SIDE STRONG"
    elif red_pct >= 65 and bull_score <= 30:
        mood, strength, signal = "TRENDING BEARISH", "STRONG", "PUT SIDE STRONG"
    elif 45 <= green_pct <= 55:
        mood, strength, signal = "CHOPPY", "WEAK", "SIDEWAYS"
    else:
        mood, strength, signal = "MIXED", "MEDIUM", "WAIT"

    if call_probability >= 75 and green_pct >= 60 and bull_score >= 70:
        final_decision, entry_advice, option_risk, color = "CALL BUY FAVOURABLE", "Buy Call only after breakout confirmation", "LOW", "#d9fbe6"
    elif put_probability >= 75 and red_pct >= 60 and put_score >= 70:
        final_decision, entry_advice, option_risk, color = "PUT BUY FAVOURABLE", "Buy Put only after breakdown confirmation", "LOW", "#ffe1e1"
    elif call_probability >= 60 and bull_score >= 55:
        final_decision, entry_advice, option_risk, color = "CALL SIDE WATCH", "Wait for breakout, avoid early entry", "MEDIUM", "#fff3cd"
    elif put_probability >= 60 and put_score >= 55:
        final_decision, entry_advice, option_risk, color = "PUT SIDE WATCH", "Wait for breakdown, avoid early entry", "MEDIUM", "#fff3cd"
    else:
        final_decision, entry_advice, option_risk, color = "NO TRADE - SIDEWAYS", "Avoid option buying in choppy market", "HIGH", "#fff3cd"

    gainers = sorted(rows, key=lambda x: x["pct"], reverse=True)[:5]
    losers = sorted(rows, key=lambda x: x["pct"])[:5]

    return render_template_string(HTML,
        nifty_price=nifty_price, final_decision=final_decision, signal=signal,
        entry_advice=entry_advice, option_risk=option_risk, color=color,
        green=green, red=red, flat=flat, green_pct=green_pct, red_pct=red_pct,
        call_probability=call_probability, put_probability=put_probability,
        bull_score=bull_score, put_score=put_score, mood=mood, strength=strength,
        weight_score=round(weight_score,2), gainers=make_table(gainers),
        losers=make_table(losers), time=datetime.now().strftime("%d-%m-%Y %H:%M:%S"))
